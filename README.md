...
### 4. 上传安全：魔数校验真实类型 + 目录白名单防路径注入

`7a703b8` 上传模块做了两层防护。第一层是**文件类型校验**：最初只校验 `file.mimetype`，但浏览器填写的 MIME 是客户端给的，改个后缀就能伪造。改成**基于文件魔数（magic bytes）检测真实类型**——读取 Buffer 的前几个字节，匹配 JPEG 的 `0xFFD8FF`、PNG 的 `0x89504E47` 等签名。**客户端声称的任何东西都不可信，包括 MIME 类型**。第二层是**目录白名单**：上传接口接收 `?directory=avatars` 参数，如果不校验，攻击者传入 `directory=../../etc` 就能让文件写到任意位置。用 `ALLOWED_DIRECTORIES = ['avatars', 'covers']` 白名单拦截，**用户可控的路径片段必须限定在预设集合内**。

### 5. 公开接口必须强制安全默认值，不能相信客户端传的过滤条件
...
### 7. 报名并发安全：同一事务内取消+递补，悲观锁内仍要做容量兜底

`aa0a4ec` 报名系统的并发安全做了三层加固。第一层是**事务边界**：取消和候补递补最初是分开的（先事务内取消，再事务外调用 `promoteWaitlisted()`），这两个操作之间哪怕只有 1ms，也可能被另一个并发取消看到「有 2 个空位」，导致双倍递补超卖。改成**取消和递补放在同一事务内，通过 `EntityManager` 传递悲观锁上下文**，锁覆盖整个业务单元。第二层是**锁内再校验**：候补递补时活动行已经上了悲观锁，但门票行没有。如果 3 个候补用户都关联同一张限量门票，直接 `soldCount += 3` 可能超卖。修复：在事务内**再锁门票行**，用 `Math.min(count, quantity - soldCount)` 计算安全递补数量。**悲观锁不是免死金牌，锁内仍要业务校验**。

### 8. N 次 COUNT 可以合并成 1 条 SUM(CASE WHEN) SQL
...
### 9. 同一个实体，内部管理和公开展示要用不同的查询方法

`aa0a4ec` 活动详情最初只有一个 `findById()`，内部管理（编辑/删除/发布）和公开展示（发现页活动卡片点入）都走同一个方法。这导致内部管理时也要考虑"是否已发布"的状态限制，或者公开展示时不小心把草稿/私密活动暴露出去。修复：**拆成两个方法**——`findById()` 不做状态/可见性限制（内部使用），`findPublicById()` 强制 `status = 'published' AND visibility = 'public'`（公开接口使用）。**安全默认值必须内置在公开查询里，而不是让调用方记得加条件**。

### 10. JOIN 会导致行重复时，子查询比 DISTINCT/GROUP BY 更干净

`aa0a4ec` 实现 `trending` 排序（按门票销量）时，最初想 JOIN `event_tickets` 表然后按 `soldCount` 排序。但一个活动有多张门票，JOIN 会导致活动行重复，需要 GROUP BY 或 DISTINCT 去重，QueryBuilder 语法变得复杂。改成**子查询** `COALESCE(SUM(ticket.soldCount), 0)` 作为标量值附加到主查询，既不重复行，又保持了 QueryBuilder 的链式清晰。**当关联表是一对多且只需要聚合值时，子查询优于 JOIN**。
